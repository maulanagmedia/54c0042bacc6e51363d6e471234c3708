package gmedia.net.id.restauranttakingorder.Utils;

/**
 * Created by Shin on 08/09/2017.
 */

public class FormatItem {

    public static final String formatDate = "yyyy-MM-dd";
    public static final String formatTimestamp = "yyyy-MM-dd HH:mm:ss";
    public static final String formatDataAndTime = "dd/MM/yyyy HH:mm";
    public static final String formatDataAndTimeStamp = "dd/MM/yyyy HH:mm:ss";
    public static final String formatTime = "HH:mm:ss";
    public static final String formatDateDisplay = "dd/MM/yyyy";
    public static final String formatTimeSummary = formatDateDisplay + "-" + formatTime;
}
